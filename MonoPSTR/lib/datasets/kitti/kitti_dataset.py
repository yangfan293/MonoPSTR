...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        self.resolution = np.array([768, 512])  #np.array([1280, 384])  # W * H
        ...
        # data split loading
        assert self.split in ['train', 'val', 'trainval', 'test', 'val_small']
        self.split_file = os.path.join(self.root_dir, 'ImageSets', self.split + '.txt')
        self.idx_list = [x.strip() for x in open(self.split_file).readlines()]

        # path configuration
        # self.data_dir = os.path.join(self.root_dir, 'testing' if split == 'test' else 'training')  # kitti
        self.data_dir = os.path.join(self.root_dir, 'validation' if split == 'val' else 'training')
    ...
    def __getitem__(self, item):
        ...
        # image encoding
        img = np.array(img)[:, :, 0:3].astype(np.float32) / 255.0  #waymo
