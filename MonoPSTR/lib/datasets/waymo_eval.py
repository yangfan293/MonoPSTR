...
class DetectionMetricsEstimatorTest(tf.test.TestCase):
    ...
    def get_boxes_from_txt(self, pd_set, gt_set, pd_dir, gt_dir):
        ...
        for i in range(len(pred_lines)): #39848
            ...
            gt_file_name  = os.path.join("/data1/hx/DEVIANT/code/data/waymo/validation/label_2/", pred_file.split('/')[-1])
            if not os.path.exists(pred_file) or not os.path.exists(gt_file_name):
                print('no exist:', gt_file_name)
                continue
    ...
    def testAPBasic(self):
        ...
        pd_bbox, pd_type, pd_frame_id, pd_score, _, gt_bbox, gt_type, gt_frame_id, gt_score, difficulty = self.get_boxes_from_txt(pd_set, gt_set, pd_dir, gt_dir)
...
if __name__ == '__main__':
    ...
    folder_path = '/data1/hx/DEVIANT/code/data/waymo/'
    # pd_set and gt_set are the validation set index, pd_set is converted to the index pattern from gt_set using set_split.py

    pd_set = "/data1/hx/DEVIANT/code/data/waymo/ImageSets/val.txt"

    gt_set = '/data1/hx/DEVIANT/code/data/waymo/ImageSets/val_org.txt' #os.path.join(folder_path, 'ImageSets/val_org.txt')

    # pd_dir is path of the predicted data for val set,

        # pd_dir = os.path.join(folder_path, 'validation/label_2')
    pd_dir = ''
    # gt_dir is the folder in which validation_org folder (generated waymo label in kitti format) is located.
    gt_dir = os.path.join(folder_path, 'validation/label_2' )
