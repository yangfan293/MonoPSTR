...
from .ops.modules import MSDeformAttn_dsp
...
class DepthAwareDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        self.cross_attn = MSDeformAttn_dsp(d_model, n_levels, n_heads, n_points)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
        self.visualanddepth = nn.Linear(d_model * 2, d_model)
        ...
        self.dropout2 = nn.Dropout(dropout)
        self.norm2 = nn.LayerNorm(d_model)
        self.depth_pos_embed = nn.Linear(1536, 50)
    ...
    def forward(self,
        ...
        # self attention
        depth_pos_embed1 = depth_pos_embed.transpose(0, 1)
        depth_pos_embed2 = depth_pos_embed1.transpose(1, 2)
        depth_pos_embed3 = self.depth_pos_embed(depth_pos_embed2).transpose(1, 2)
        v = depth_pos_embed3.transpose(0, 1)
        q = k = self.with_pos_embed(depth_pos_embed3.transpose(0, 1), query_pos.transpose(0, 1))
        tgt2 = self.self_attn(q, k, v)[0].transpose(0, 1)
        tgt2 = tgt + tgt2
        tgt = tgt + self.dropout2(tgt2)
        tgt = self.norm2(tgt)

        # visual cross attention
        q = torch.cat([tgt, query_pos], dim=2)
        q_cat = self.visualanddepth(q)
        tgt2 = self.cross_attn(q_cat,
                               reference_points,
                               src, src_spatial_shapes, level_start_index, src_padding_mask, refHW_cond)
...
class DepthAwareDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, d_model=None, use_dab=False, two_stage_dino=False):
        ...
        if use_dab:
            ...
            self.ref_point_head = MLP(2 * 256, 256, 256, 2)
            self.ref_anchor_head = MLP(256, 256, 2, 2)
            self.ref_liner = nn.Linear(2, 2)
        else:
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            query_pos_un=None
            if self.use_dab:
                query_sine_embed = gen_sineembed_for_position(reference_points_input[:, :, 0, :2])  # bs, nq, 256*2
                pos_scale = self.query_scale(output) if lid != 0 else 1
                refHW_cond = self.ref_anchor_head(output).sigmoid()  # nq, bs, 2
                query_pos = pos_scale * query_sine_embed
            ...
            output = layer(output,
                           ...
                           is_first=(lid == 0), depth_pos_embed_ip=depth_pos_embed_ip, pos_embeds=pos_embeds, self_attn_mask=attn_mask,query_pos_un=query_pos_un, refHW_cond=refHW_cond)
