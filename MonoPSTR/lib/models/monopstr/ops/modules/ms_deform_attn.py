...
class MSDeformAttn_dsp(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None, refHW_cond=None):
        """
        :param query                       (N, Length_{query}, C)
        :param reference_points            (N, Length_{query}, n_levels, 2), range in [0, 1], top-left (0,0), bottom-right (1, 1), including padding area
                                        or (N, Length_{query}, n_levels, 4), add additional (w, h) to form reference boxes
        ...
        if reference_points.shape[-1] == 2:
            ...
            sampling_locations = reference_points[:, :, None, :, None, :] \
                                 + sampling_offsets / offset_normalizer[None, None, None, :, None, :]
        elif reference_points.shape[-1] == 6:   # 8,5,4,6
            w_pre = reference_points[...,0, 2] + reference_points[...,0, 3]
            h_pre = reference_points[...,0, 4] + reference_points[...,0, 5]  # 8,5,4
            sampling_offsets_refine = sampling_offsets.clone()
            w_rel = (w_ref / w_pre).unsqueeze(-1) #8,50,1
            h_rel = (h_ref / h_pre).unsqueeze(-1)
            sampling_offsets_refine[..., 0] = sampling_offsets[..., 0] * w_rel[:, :, None, None, None, 0]
            sampling_offsets_refine[..., 1] = sampling_offsets[..., 1] * h_rel[:, :, None, None, None, 0]
            sampling_locations = reference_points[:, :, None, :, None, :2] \
                                 + sampling_offsets_refine / self.n_points * (reference_points[:, :, None, :, None, 2::2] + reference_points[:, :, None, :, None, 3::2]) * 0.5
        else:
            raise ValueError(
                'Last dim of reference_points must be 2 or 4, but get {} instead.'.format(reference_points.shape[-1]))
        output = MSDeformAttnFunction.apply(
